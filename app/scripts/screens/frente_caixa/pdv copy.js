...
function startPDV(date) {
    ...
    new Vue({
        ...
        methods: {
            ...
            validity: function () {
                if (this.rootus.date_now === moment().format("YYYY-MM-DD")) {
                    let validity = this.rootus.validity;
                    let now = this.rootus.date_now;
            ...
            adjustValuesvUnCom: function (val) {
                ...
                var regExp0_2 = /^\d\d$/;
                var regExp0_3 = /^\d\d\d$/;
                var regExp0_4 = /^\d\d\d\d$/;
                var regExp1 = /^\d\.\d\d$/;
                var regExp1_2 = /^\d\d\.\d\d$/;
                var regExp1_3 = /^\d\d\d\.\d\d$/;
                var regExp1_4 = /^\d\d\d\d\.\d\d$/;
            ...
            /**
             *----------------------------------------------------
             * Calcula os impostos da nota baseado no ICMSSN102.
             *----------------------------------------------------
             */
            adicionarTotalNaNotaICMSSN102: function () {
                ...
                console.log(this.nota.enviNFe.NFe.pag.vPag);
                console.log(this.nota.enviNFe.NFe.total.ICMSTot.vNF);


                let regExp0 = /^\d{1,4}\.\d$/;


                console.log(regExp0.test(this.nota.enviNFe.NFe.total.ICMSTot.vNF));
            ...
            finishRegister: function () {
                ...
                if (this.venda.listProduto !== 0) {
                    if (confirm("Tem certeza que deseja finalizar a compra?") == true) {
                        ...
                        /**
                         *----------------------------------------------------
                         * Calcula os impostos da nota. Pode ser trocado.
                         *----------------------------------------------------
                         */
                        if (this.empresas[0].icmstipo == '102') {
                            console.log('Tipo ICMS: ', 102);
                            this.adicionarTotalNaNotaICMSSN102();
                        } else {
                            console.log('Tipo ICMS: ', 'A Definir');
            ...
            toQtde: function () {
                ...
                if (this.search == '') {
                    dialog.showErrorBox('Por favor insira um dado para começar a pesquisa de produto!');
                    this.focusInProductField();
                }
                else {
                    document.getElementById("qtdeInFocus").focus();
            ...
            toDiscount: function () {
                ...
                if (this.venda.produto[0].quantidade == '') {
                    dialog.showErrorBox('Por favor insira uma quantidade!');
                    document.getElementById("qtdeInFocus").focus();
                }
                else if (this.venda.produto[0].quantidade == 0 || this.venda.produto[0].quantidade < 0) {
                    dialog.showErrorBox('Este campo não pode ser preenchido com valores menores que zero ou iguais a zero!');
                    document.getElementById("qtdeInFocus").focus();
                }
                else {
            ...
            toValueReceived: function () {
                ...
                /**
                 *----------------------------------------------
                 * This message is to field discount.
                 *----------------------------------------------
                 */
